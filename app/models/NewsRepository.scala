...
trait NewsRepository {
  ...
  def updateArticleStatus(id: Int, status: Content.Status)(implicit userOption: Option[User]): Future[ArticleInflated]
  ...
  def updateArticleTemplateStatus(id: Int, status: Content.Status)(implicit userOption: Option[User]): Future[ArticleTemplate]
}
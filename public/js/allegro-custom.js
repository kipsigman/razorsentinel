function printArticle() {
  var html = jQuery(".block-content .shortcode-content .paragraph-row").html();
  var htmltitle = jQuery(".article-title h1").html();
  if(jQuery(".main-article-content .article-photo img").size() > 0){var htmlphoto = "<img src='"+jQuery(".main-article-content .article-photo img").attr("src")+"' alt='' style='max-width: 600px;'/>";}else{var htmlphoto = "";}
  var htmlLogo = '<div class="logo-image">' + jQuery(".header-logo").html() + '</div>';
  ...
  top.consoleRef.document.writeln(
    ...
    +htmlLogo
    +'<h2>'+htmltitle+'</h2>'
    +'<div class="smallinfo"><span>'+htmldate+'</span> <a href="'+jQuery(location).attr('href')+'">'+jQuery(location).attr('href')+'</a></div>'
